...
export function MenuDishCard({ item, index, categoryLabel = "мангал" }: MenuDishCardProps) {
    ...
    <>
      ...
      >
        <button
          ...
          className="menu-dish-card__media"
        ...
        >
          ...
          <div className="menu-dish-card__media-top" aria-hidden>
            {item.badge && (
              <span className={`badge ${item.badge === "хит" ? "badge-hit" : "badge-gold"}`}>
                {item.badge}
              </span>
            )}
        ...
        <div className="menu-dish-card__body">
          ...
          <div className="menu-dish-card__footer">
            <div className="menu-dish-card__price">
              <span className="menu-dish-card__price-value">
                {item.price.toLocaleString("ru-RU")}
              </span>
              <span className="menu-dish-card__price-currency">₽</span>
            </div>
            <div className="menu-dish-card__actions">
              <button type="button" onClick={() => setOpen(true)} className="menu-dish-card__ghost">
                Читать
              </button>
              <button
                type="button"
        ...
        {open && (
          <>
            ...
            <motion.div
              role="dialog"
              aria-modal="true"
              aria-labelledby={`dish-title-${item.name}`}
            ...
            >
              ...
              <div className="menu-dish-modal__content">
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  {item.unit && (
                    <span className="label-caps text-ink/35">{item.unit}</span>
                  )}
                </div>

                <h3
                  id={`dish-title-${item.name}`}
                  className="menu-dish-modal__title"
                >
                  {item.name}
                </h3>
                ...
                <div className="menu-dish-modal__bar">
                  <div className="menu-dish-card__price">
                    <span className="menu-dish-card__price-value text-[clamp(1.75rem,4vw,2.25rem)]">
                      {item.price.toLocaleString("ru-RU")}
                    </span>
                    <span className="menu-dish-card__price-currency">₽</span>
                  </div>
